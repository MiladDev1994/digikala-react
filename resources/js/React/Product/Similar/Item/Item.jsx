...
const Item = ({item , index}) => {
    ...
    return (
        <Link
            ...
            to={href ? `http://127.0.0.1:8000/product/${item.product[0].id}` : null}
            target={href ? '_blank' : null}
            className={`${index === 0 ? 'rounded-start' : index === 19 ? 'rounded-end' : ''} border-end border-secondary border-opacity-25 p-2 h-100 position-absolute text-light ${styles.items}`}
        ...
        >
            ...
            <div className={`${styles.detail} mt-4`}>
                ...
                {
                    item.price !== item.price_off ?
                        <div className={`${styles.price} text-center mt-2 opacity-25`}>{item.price}</div>
                        : null
                }
            </div>
        </Link>
