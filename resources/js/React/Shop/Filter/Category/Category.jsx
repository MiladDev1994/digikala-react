        ...
        <div className={'w-100 text-light '}>
            ...
            <div className={`${accordion ? styles.listShow : styles.listHidden} border-bottom border-secondary border-opacity-25`}>
                ...
                <div className={`${styles.Data} p-2`}>
                    {categories.length ?
                        filter.map(item =>
                            <div key={item.id} className={`position-relative p-2 mt-1 rounded-3 opacity-75 ${styles.item} ${query.category.includes(item.name) ? 'bg-primary' : ''}`}>
                                <input type={'checkbox'} checked={query.category.includes(item.name) ? true : false} name={'category'} value={item.name} onChange={queryStringHaandler} className={'position-absolute w-100 h-100 opacity-0'} style={{top: 0 , cursor:'pointer'}} />
                                {item.name}
                            </div>
                        )
                        :null
